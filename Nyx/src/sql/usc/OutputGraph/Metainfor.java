...
public class Metainfor {
	...
	public void LoadLibraries(String libpath){

	}
	public Metainfor(MIF mif)  throws WAMConfigurationException, ClassFormatException, IOException
	{
		...
		File libfolder=new File(mif.libpath);
		if(!libfolder.exists() || !libfolder.isDirectory())
		{
			throw new Error(" the libfodler doesnot exist or is a folder");
		}
		File[] listOfFiles = libfolder.listFiles();

		for (int i = 0; i < listOfFiles.length; i++) {
			sr.addClassPath(listOfFiles[i].getAbsolutePath());
		...
		WAMConfiguration conf=WAMConfiguration.load(mif.classconfig);
		...
		Utils.Log("call graph is built"+(time2-time1));
		...
		Utils.Log("Strnig analysis finished"+(time3-time2));
	}
